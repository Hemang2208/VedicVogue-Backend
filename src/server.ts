...
const setupSelfPingCron = (): void => {
  ...
  if (!renderUrl) {
    console.warn('⚠️  RENDER_SERVICE_URL or BASE_URL not found. Self-ping CRON job not started.');
    return;
  }

  // CRON job: runs every 5 minutes to keep the server alive
  cron.schedule("*/5 * * * *", async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`⚠️  Self-ping returned status: ${response.status} at ${new Date().toISOString()}`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error(`❌ Self-ping failed at ${new Date().toISOString()}:`, errorMessage);
    }
  });

  console.log(`🔄 Self-ping CRON job started - pinging ${renderUrl}/health every 5 minutes`);
};
